...
- (id)initWithPoint:(CGPoint)point hue:(CGFloat)hue
{
    const CGFloat kFingerRadius = 30.0f;//22.0f;
    ...
    if ((self = [super initWithFrame:CGRectMake(point.x-kFingerRadius, point.y-kFingerRadius, 2*kFingerRadius, 2*kFingerRadius)]))
    {
        ...
        //self.layer.backgroundColor = [UIColor colorWithHue:hue saturation:0.5f brightness:0.5f alpha:0.5f].CGColor;
        self.layer.backgroundColor = [UIColor colorWithHue:199.0f/360.0f saturation:0.94f brightness:0.99f alpha:0.5f].CGColor;
    }
    return self;
}
